...
@section('page_title') Add Qestion @endsection
    ...
    <div class="page-header mt-3">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">Add a Question</li>
            </ol>
        </nav>
    </div>
@endsection
    ...
    <form class="forms-sample" action="{{ route('questions.store') }}" method="POST">
        ...
        <div class="row">
            ...
            <div class="form-group col-md-6">
                ...
                <input type="text" name="question_title" id="question_title" class="form-control" value="{{ old('question_title') }}"
            ...
            <div class="form-group col-md-6">
                ...
                <select  name="question_status" id="status" class="form-control form-control-lg">
                    ...
                    <option value="1" {{ old('question_status') == '1' ? 'selected' : '' }}>Active</option>
                    <option value="0" {{ old('question_status') == '0' ? 'selected' : '' }}>Inactive</option>
                </select>
                @if ($errors->has('question_status'))
                    <div class="alert alert-danger">{{ $errors->first('question_status') }}</div>
                @endif
            </div>
            <div class="form-group col-md-6">
                <label for="no_awnsers">No of Awnsers</label>
                <input type="text"  name="no_awnsers" id="no_awnsers" class="form-control"  value="{{ old('no_awnsers') }}" />
            ...
            <div class="form-group col-md-6">
                ...
                <textarea name="question_description" id="question_description" class="form-control" rows="4">{{old('description')}}</textarea>
